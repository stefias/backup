...
class SimpleDynamicIsland(QWidget):
    def __init__(self):
        ...
        self.base_expanded_w, self.base_expanded_h = 420, 210  # Made slightly larger/longer
    ...
    def setup_ui(self):
        ...
        # Setup opacity effects for smooth 0.25s (250ms) fade out/in of expanded elements
        self.fade_anims = []
        for w in [self.btn_prev, self.btn_play, self.btn_next, self.title_label, self.artist_label, self.time_left, self.time_right]:
            effect = QGraphicsOpacityEffect(w)
            effect.setOpacity(1.0)
            w.setGraphicsEffect(effect)

    def fade_ui_elements(self, fade_in=True, duration=250):
        elements = [self.btn_prev, self.btn_play, self.btn_next, self.title_label, self.artist_label, self.time_left, self.time_right]
        target = 1.0 if fade_in else 0.0
        self.fade_anims = []
        for el in elements:
            effect = el.graphicsEffect()
            if effect:
    ...
    def paintEvent(self, event):
        ...
        # Liquid glass background replacement
        glass_gradient = QLinearGradient(self.margin, self.margin, self.margin, self.margin + current_h)
        glass_gradient.setColorAt(0.0, QColor(45, 45, 60, 190))
        glass_gradient.setColorAt(1.0, QColor(20, 20, 30, 210))
    ...
    def mousePressEvent(self, event):
        ...
        if not self.is_expanded:
            ...
            # Fade in expanded controls over 0.25 seconds (250ms)
            for w_el in [self.btn_prev, self.btn_play, self.btn_next, self.title_label, self.artist_label, self.time_left, self.time_right]:
                if w_el.graphicsEffect():
                    w_el.graphicsEffect().setOpacity(0.0)
            self.fade_ui_elements(fade_in=True, duration=250)
        else:
            w, h = self.collapsed_w, self.collapsed_h
            self.is_expanded = False
            # Fade out expanded controls over 0.25 seconds (250ms)
            self.fade_ui_elements(fade_in=False, duration=250)
    ...
    def show_expanded_ui(self):
        ...
        self.title_label.setGeometry(self.margin + 115, self.margin + 28, 220, 24)
        ...
        self.artist_label.setGeometry(self.margin + 115, self.margin + 54, 220, 24)
        self.artist_label.show()
        
        self.wave_widget.setGeometry(self.expanded_w - self.margin - 55, self.margin + 35, 35, 25)
        self.wave_widget.show()
        
        self.time_left.setGeometry(self.margin + 25, self.margin + 125, 45, 20)
        self.time_left.show()
        
        self.time_right.setGeometry(self.expanded_w - self.margin - 70, self.margin + 125, 45, 20)
        self.time_right.show()
        
        self.btn_prev.setGeometry(self.margin + 115, self.margin + 160, 44, 28)
        self.btn_play.setGeometry(self.margin + 190, self.margin + 154, 40, 40)
        self.btn_next.setGeometry(self.margin + 265, self.margin + 160, 44, 28)
