...
class SpringDynamicIsland(QWidget):
    ...
    def __init__(self):
        ...
        # Start disabled initially, will trigger on toggle/start
        ...
        # Initialize UI positions at collapsed state (p = 0.0)
    ...
    def update_animation(self):
        ...
        # If spring has settled, clamp to exact target and stop timer completely (like a statue)
    ...
    def update_widget_geometries(self, p):
        ...
        # 6. Media Control Buttons (FIXED ANIMATION)
        # By removing the 'p' multiplier from the X coordinates, the buttons
        # drop straight vertically rather than sliding in diagonally.
        prev_x = self.margin + 107
        prev_y = int(
            (self.margin + 20) + ((self.margin + 150) - (self.margin + 20)) * p
        )
        self.btn_prev.setGeometry(prev_x, prev_y, 44, 28)

        play_x = self.margin + 180
        play_y = int(
            (self.margin + 20) + ((self.margin + 144) - (self.margin + 20)) * p
        )
        self.btn_play.setGeometry(play_x, play_y, 40, 40)

        next_x = self.margin + 253
        next_y = int(
            (self.margin + 20) + ((self.margin + 150) - (self.margin + 20)) * p
        )
        self.btn_next.setGeometry(next_x, next_y, 44, 28)
    ...
    def paintEvent(self, event):
        ...
        if self.is_expanded or p > 0.05:
            ...
            if self.track_length > 0:
                # Safely clamp progress fill within bounds to prevent jitter/glitching
