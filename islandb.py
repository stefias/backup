...
class SpringDynamicIsland(QWidget):
    ...
    def update_widget_geometries(self, p):
        ...
        # 3. Title Label (Constrained inside island bounds)
        ...
        self.title_label.setStyleSheet(
            f"color: rgba(255, 255, 255, {max(0.0, min(1.0, p))});"
        )

        # 4. Artist Label (Constrained inside island bounds)
        ...
        self.artist_label.setStyleSheet(
            f"color: rgba(187, 187, 187, {max(0.0, min(1.0, p))});"
        )

        # 5. Time Labels
        tl_x = int((self.margin + 20) + ((self.margin + 25) - (self.margin + 20)) * p)
        tl_y = int(
            (self.margin + 100) + ((self.margin + 120) - (self.margin + 100)) * p
        )
        self.time_left.setGeometry(tl_x, tl_y, 45, 20)
        self.time_left.setStyleSheet(
            f"color: rgba(224, 224, 224, {max(0.0, min(1.0, p))});"
        )

        collapsed_tr_x = self.collapsed_w - self.margin - 65
        expanded_tr_x = self.expanded_w - self.margin - 70
        tr_x = int(collapsed_tr_x + (expanded_tr_x - collapsed_tr_x) * p)
        tr_y = int(
            (self.margin + 100) + ((self.margin + 120) - (self.margin + 100)) * p
        )
        self.time_right.setGeometry(tr_x, tr_y, 45, 20)
        self.time_right.setStyleSheet(
            f"color: rgba(224, 224, 224, {max(0.0, min(1.0, p))});"
        )

        # 6. Media Control Buttons (Horizontally Centered & Vertical Movement)
        # Total width = 44 (prev) + 40 (play) + 44 (next) + 32 (two 16px spacings) = 160
        # Center of 400px inner area is 200. Start X = 200 - 80 = 120.
        group_start_x = 120
        prev_local_x = group_start_x
        play_local_x = group_start_x + 44 + 16
        next_local_x = play_local_x + 40 + 16

        prev_x = self.margin + prev_local_x
        play_x = self.margin + play_local_x
        next_x = self.margin + next_local_x

        # Up when minimizing (p -> 0), Down when expanding (p -> 1)
        collapsed_y = self.margin + 10
        expanded_y_buttons = self.margin + 145

        btn_y = int(collapsed_y + (expanded_y_buttons - collapsed_y) * p)

        self.btn_prev.setGeometry(prev_x, btn_y, 44, 28)
        self.btn_play.setGeometry(play_x, btn_y - 6, 40, 40)
        self.btn_next.setGeometry(next_x, btn_y, 44, 28)
    ...
    def poll_media(self):
        ...
        try:
            ...
            # Clean artist handling (joined with commas instead of slashes)
            artists = metadata.get("xesam:artist", ["Unknown"])
            if isinstance(artists, (dbus.Array, list)):
                new_artist = ", ".join(str(a) for a in artists) if artists else "Unknown"
            else:
                new_artist = str(artists) if artists else "Unknown"
    ...
    def paintEvent(self, event):
        ...
        if self.is_expanded or p > 0.05:
            ...
            bar_alpha = int(255 * p)
            painter.setBrush(QColor(255, 255, 255, int(60 * p)))
            painter.setPen(Qt.NoPen)
            painter.drawRoundedRect(bar_x, bar_y, bar_w, bar_h, 3, 3)

            if self.track_length > 0:
                safe_pos = max(0, min(self.current_position, self.track_length))
                fill_w = int((safe_pos / self.track_length) * bar_w)
