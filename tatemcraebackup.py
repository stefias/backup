...
class Spring:
    ...
    def step(self, dt):
        ...
        self.value += diff * min(1.0, 9.5 * dt)  # Slightly gentler interpolation speed
...
class AnimatedWaveform(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.is_playing:
            t = time.time() * 2.8  # Slightly increased speed from 2.0
            self.heights = []
            for i in range(5):
                # Reduced amplitude for a less intense pulse
...
class IconButton(QPushButton):
    ...
    def __init__(self, icon_type, parent=None):
        ...
        # Click blur animation support for click feedback on all buttons
        self.click_anim_val = 0.0
        self.click_timer = QTimer(self)
        self.click_timer.timeout.connect(self._step_click_anim)

        # Graphics blur effect for transition morphing & click feedback blur
    ...
    def _step_click_anim(self):
        self.click_anim_val -= 1.0 / 30.0  # Fades out over ~0.30 seconds (30 steps * 10ms)
    ...
    def _step_anim(self):
        self.anim_val += 1.0 / 30.0  # Transitions over ~0.30 seconds (30 steps * 10ms)
    ...
    def update_blur(self):
        # Minimized transition blur peak intensity slightly (from 2.0 to 1.8)
        trans_blur = 0.0
        if self.anim_timer.isActive() and self.old_icon_type != self.icon_type:
            trans_blur = 1.8 * math.sin(self.anim_val * math.pi)

        # Minimized click blur max intensity slightly (from 2.5 to 2.2 pixels)
    ...
    def paintEvent(self, event):
        ...
        if self.anim_val >= 1.0 or self.old_icon_type == self.icon_type:
            ...
            out_scale = 1.0 - (0.1 * self.anim_val)  # Slightly less scaling intensity
...
class SpringDynamicIsland(QWidget):
    ...
    def update_widget_geometries(self, p):
        ...
        # Update custom MarqueeLabel text color and opacity
    ...
    def paintEvent(self, event):
        ...
        for i in range(3, 0, -1):
            ...
            alpha = int(30 / i)  # Softer, less intense shadow
