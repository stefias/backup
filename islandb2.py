...
class Spring:
    """Mass-spring-damper: tuned with an underdamped setup for a smooth elastic bounce at the end."""

    def __init__(self, stiffness=130.0, damping=16.0, mass=1.0):
        self.k = stiffness
        self.c = damping
        self.m = mass
        self.value = 0.0
    ...
    @property
    def settled(self):
        return abs(self.velocity) < 0.05 and abs(self.target - self.value) < 0.0005
...
class SpringDynamicIsland(QWidget):
    ...
    def __init__(self):
        ...
        self.p_spring = Spring(stiffness=130.0, damping=16.0, mass=1.0)
    ...
    def update_animation(self):
        ...
        # Clamp width transition so horizontal size stays clean/stable
        p_clamped = max(0.0, min(1.0, p))
        w = self.collapsed_w + (self.expanded_w - self.collapsed_w) * p_clamped
        
        # Let height use raw bouncy p so the bottom edge stretches and drops down with a spring effect
        h = self.collapsed_h + (self.expanded_h - self.collapsed_h) * p
        
        # Anchor the top edge completely still (y stays at y_pos)
    ...
    def update_widget_geometries(self, p):
        # Use a stiff/clamped p for internal elements so they don't bounce/warp
        p_stiff = max(0.0, min(1.0, p))
        ...
        self.title_label.setStyleSheet(
            f"color: rgba(255, 255, 255, {alpha_p});"
        )
